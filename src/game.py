...
import math
from modsSidebar import ModsSidebar
...
class Arjun(pygame.Surface):
    ...
    def tick(self, dt, camera: ui.CGCamera):
        dt /= 1000  # because its in seconds
        ...
        # angle calculations
        try:
            self.draw_degree = -(math.degrees(
                math.atan(self.arjun.pos[1] + self.force[1] / self.arjun.pos[0] - self.force[0])) * 1000) % 360
...
class Game(menu.MenuType):
    def __init__(self, screen: ui.CScreen, settings: config.Settings):
        ...
        self.camera = ui.CGCamera(self.screen.surface)
        ...
        self.SIDEBAR_mods = ModsSidebar(settings, (600, 0), self)
        ...
        def launch():
            ...
            self.arjun.force = self.starterForce[:]  # [:] copies the list to prevent references.
    ...
    def run(self):
        ...
        while True:
            ...
            self.SIDEBAR_mods.manager.tick(events)
            ...
            if self.SIDEBAR_mods.show:
                self.SIDEBAR_mods.draw(self.screen)
            else:
                ...
                if self.settings.DEBUG:
                    ...
                    self.screen.draw(self.LABEL_vals)
